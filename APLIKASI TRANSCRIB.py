...
# =====================================================================
st.set_page_config(
    page_title="TranscribX - Enterprise AI",
    page_icon="🎙️",
    layout="wide"
)
...
def apply_custom_css():
    """Menerapkan CSS kustom untuk menyembunyikan toolbar Streamlit"""
...
def initialize_firebase():
    """Inisialisasi koneksi Firebase Admin SDK"""
...
db = firestore.client() if firebase_admin else None
...
LITELLM_URL = "https://litellm.koboi2026.biz.id"

# =====================================================================
# FUNGSI AUTHENTIKASI FIREBASE
# =====================================================================
def login_firebase(email, password):
    """Login ke Firebase Authentication menggunakan REST API"""
...
def register_firebase(email, password):
    """Register akun baru ke Firebase Authentication"""
    api_key = st.secrets["FIREBASE_WEB_API_KEY"]
...
def check_subscription(uid):
    """Memeriksa status subscription user dari Firestore"""
    if not db:
...
def update_user_kuota(uid, kuota_ai=None, kuota_upload=None):
    """Update kuota user di Firestore"""
    if not db:
        return False
    ...
    try:
        ...
        if kuota_ai is not None:
            update_data["kuota_ai"] = kuota_ai
        if kuota_upload is not None:
            update_data["kuota_upload"] = kuota_upload
        ...
        st.error(f"Error updating kuota: {e}")
        return False

# =====================================================================
# FUNGSI LITELLM API
# =====================================================================
def call_litellm_transcription(api_key, audio_file):
    """Mentranskripsi audio menggunakan LiteLLM Whisper"""
...
def call_litellm_summary(api_key, transcript_text):
    """Generate AI Summary menggunakan LiteLLM Gemini"""
...
# =====================================================================
# FUNGSI SIDEBAR GERMIC ROBOT
# =====================================================================
def render_germic_sidebar():
    """Menampilkan animasi robot Germic di sidebar"""
    st.sidebar.markdown("<h3 style='text-align: center; color: #475569;'>🤖 AI Assistant</h3>", unsafe_allow_html=True)
    ...
    <script>
        ...
        function trackMouse(clientX, clientY, screenWidth) {
            if (!face) return;
            const robotX = screenWidth * 0.2;
            const robotY = 125;
            const mouseX = clientX - robotX;
            const mouseY = clientY - robotY;
            const limit = 8;
        ...
        try {
            window.parent.document.addEventListener('mousemove', (e) => trackMouse(e.clientX, e.clientY, window.parent.innerWidth));
        } catch (err) {}
    </script>
    """
    components.html(germic_html, height=250)
    st.sidebar.markdown(
        "<p style='text-align: center; color: #94a3b8; font-size: 13px; font-weight: bold;'>GERMIC System Online</p>",
        unsafe_allow_html=True
    )
    st.sidebar.markdown("---")

def render_user_info_sidebar():
    """Menampilkan informasi user di sidebar"""
    if st.session_state.get("logged_in"):
        st.sidebar.markdown(
            f"**👤 Pengguna:**<br><span style='font-size:12px;'>{st.session_state.get('user_email')}</span>",
            unsafe_allow_html=True
        )
        st.sidebar.markdown(f"**🏷️ Paket:** {st.session_state.get('user_paket', 'NON-AKTIF')}")
        ...
        if st.session_state.get('user_paket') != 'NON-AKTIF':
            ...
            st.sidebar.markdown(
                f"**✨ Sisa AI Summary:** <span style='color:{ai_color}; font-weight:bold;'>{st.session_state.get('user_kuota_ai', 0)}x</span>",
                unsafe_allow_html=True
            )
            st.sidebar.markdown(
                f"**📁 Sisa Upload Audio:** <span style='color:{up_color}; font-weight:bold;'>{st.session_state.get('user_kuota_upload', 0)}x</span>",
                unsafe_allow_html=True
            )
        st.sidebar.markdown("---")
...
def render_login_page():
    ...
    with col2:
        ...
        st.markdown(
            "<p style='text-align: center; color: #64748b;'>Gunakan kredensial Anda untuk mengakses sistem Notulensi AI.</p>",
            unsafe_allow_html=True
        )
...
# =====================================================================
# FUNGSI TAB INFO PAKET
# =====================================================================
def render_package_info_tab():
    """Menampilkan informasi paket langganan"""
...
# =====================================================================
# FUNGSI TAB LIVE TRANSCRIPTION
# =====================================================================
def render_live_transcription_tab():
    """Menampilkan tab live transcription dengan Web Speech API"""
    ...
    <body>
        ...
        <script>
            ...
            window.exportCyToPng = function() {
                if (!myCyInstance) return alert("Cytoscape belum siap!");
                const b64 = myCyInstance.png({ full: true, scale: 4, bg: 'white' });
            ...
            window.exportNotulensiTxt = function() {
                if (!lastAiData) return alert("Data Notulensi belum ada!");
                const root = lastAiData;
                ...
                txt += `\\nAgenda: ${data.agenda || '-'}\\n`;
                txt += `Peserta: ${data.peserta ? data.peserta.join(', ') : '-'}\\n\\n`;
                ...
                if (data.rencana_tindak_lanjut && data.rencana_tindak_lanjut.length > 0) {
                    data.rencana_tindak_lanjut.forEach(t => {
                        txt += `- [${t.prioritas}] ${t.tugas} (PIC: ${t.pic}, Deadline: ${t.deadline})\\n`;
                    });
                } else {
            ...
            if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
                    ...
                    try {
                        ...
                        mediaRecorder.onstop = () => {
                            ...
                            audioItem.innerHTML = `
                                ...
                                <a href="${audioUrl}" download="${fileName}" class="btn-custom" style="background:#10b981;">💾 Download Audio</a>
                            `;
                        ...
                        status.innerText = "Izin Mic Ditolak!";
                    }
                };
                ...
                downloadTxtBtn.onclick = () => {
                    ...
                    const a = document.createElement('a');
                    a.href = URL.createObjectURL(blob);
                    a.download = 'Transkrip_Raw_' + new Date().getTime() + '.txt';
                    a.click();
                };
                ...
                aiBtn.onclick = async () => {
                    ...
                    if (!transcript) {
                        alert("⚠️ Transkrip masih kosong.");
                        return;
                    }
                    ...
                    aiContent.innerHTML = `
                        <div class="p-8 bg-purple-50 rounded-[2.5rem] border border-purple-200 shadow-sm text-center fade-in mt-6">
                            <p class="text-purple-600 font-bold animate-pulse">Memproses Notulensi, Cytoscape, Markmap & Mermaid... Mohon tunggu (±15 detik).</p>
                        </div>
                    `;
...
Transkrip Rapat: "${transcript}"`;
                    ...
                    try {
                        ...
                        if (resJson.choices && resJson.choices[0].message.content) {
                            ...
                            // Build UI...
                            // [Kode render UI sama seperti sebelumnya]
...
# =====================================================================
# FUNGSI TAB OFFLINE TRANSCRIPTION
# =====================================================================
def render_offline_transcription_tab():
    """Menampilkan tab offline transcription"""
    ...
    if uploaded_file is not None:
        if uploaded_file.size > 26214400:  # 25MB
                ...
                if st.button("🎙️ Mulai Transkripsi (via LiteLLM Whisper)", use_container_width=True, type="primary"):
                    if not llm_key:
                        ...
                        with st.spinner("⏳ Sedang mentranskripsi audio..."):
                            try:
                                response = call_litellm_transcription(llm_key, uploaded_file)
                                
                                if hasattr(response, 'status_code') and response.status_code == 200:
                                    st.session_state["offline_transcript"] = response.json().get("text", "")
                                    
                                    # Kurangi kuota jika bukan admin
                                    if not is_admin:
                                    ...
                                    st.error(f"❌ Error dari API LiteLLM: {error_msg}")
                            except Exception as e:
    ...
    if st.session_state["offline_transcript"]:
        ...
        transcript_area = st.text_area(
            "Edit jika perlu sebelum di-Summary:",
            value=st.session_state["offline_transcript"],
            height=250
        )
            ...
            if st.button("✨ Generate AI Summary dari Teks Ini", use_container_width=True, type="secondary"):
                if not llm_key:
                    ...
                    with st.spinner("⏳ AI sedang memproses JSON Notulensi & Visual..."):
                        try:
                            response = call_litellm_summary(llm_key, st.session_state["offline_transcript"])
                            
                            if hasattr(response, 'status_code') and response.status_code == 200:
                                st.session_state["offline_summary"] = json.loads(
                                    response.json()["choices"][0]["message"]["content"]
                                )
                                
                                # Kurangi kuota AI jika bukan admin
                                if not is_admin:
                                ...
                                error_msg = response.text if hasattr(response, 'text') else str(response)
                                st.error(f"Error AI: {error_msg}")
                        except Exception as e:
                            st.error(f"Koneksi LLM Gagal: {str(e)}")
    
    # Tampilkan hasil summary
    if st.session_state["offline_summary"]:
        render_summary_results(st.session_state["offline_summary"])

# =====================================================================
# FUNGSI RENDER SUMMARY RESULTS
# =====================================================================
def render_summary_results(data):
    """Menampilkan hasil summary dalam UI yang rapi"""
    st.markdown("---")
    ...
    # Generate TXT report for download
    ...
    for d in data['notulensi_rapat'].get('jalannya_diskusi', []):
        txt_report += f"- {d}\n"
    txt_report += "\nKeputusan Utama:\n"
    for k in data['notulensi_rapat'].get('keputusan', []):
        txt_report += f"- {k}\n"
    txt_report += "\nRencana Tindak Lanjut:\n"
    ...
    with col_t2:
        st.download_button(
            ...
            data=txt_report,
            file_name="Notulensi_Offline.txt",
            mime="text/plain",
    ...
    with st.container(border=True):
        ...
        for r in data.get('ringkasan_eksekutif', []):
            rx_html += f"<li style='margin-bottom:5px;'>{r}</li>"
        rx_html += "</ul></div>"
        ...
        for d in data['notulensi_rapat'].get('jalannya_diskusi', []):
            diskusi_html += f"<li style='margin-bottom:8px;'>{d}</li>"
        diskusi_html += "</ul></div>"
    ...
    # Visualisasi
    st.markdown("### 🕸️ Visualisasi (Hover/Klik Tombol PNG di Kanan Atap)")
    ...
    with col_v1:
        ...
        <body style="margin:0; padding:10px; background:#f8fafc; border-radius:12px; font-family: sans-serif; position:relative;">
            ...
            <script>
                ...
                function dlCy() {{
                    const a = document.createElement('a');
                    a.href = cy.png({{full: true, scale: 4, bg: 'white'}});
                ...
                try {{
                    ...
                    setTimeout(() => {{
                        html2canvas(container, {{
                            ...
                            const link = document.createElement('a');
                            link.download = `MindMap_${{title}}.png`;
                            link.href = canvas.toDataURL('image/png', 1.0);
                            link.click();
...
def main():
    ...
    if is_admin:
        ...
        with tabs[2]:
            render_offline_transcription_tab()
        with tabs[3]:
            render_package_info_tab()
    else:
